...
#'
#' @description
#' \code{\link[=tuav_loc]{tuav_loc()}} gives the location of all images with the option to retrieve the extents overlap. Furthermore, it gives the option to export the GPS/extent information
#'
...
#' @return an updated \code{\link[=ThermalUAV-Class]{ThermalUAV}} object containing additional positional information
#' @seealso \code{\link[=tuav_create]{tuav_create()}}, \code{\link[=tuav_view]{tuav_view()}}
#' @export
tuav_loc <- function(thermal_uav,
                     extent = FALSE,
                     overlap = FALSE,
                     export = FALSE){
  ...
  if ((thermal_uav@Info@camera == "ThermalCapture") & (length(thermal_uav@Info@meta_df) != 0) &
    ...
    suppressWarnings({
      ...
      if (length(thermal_uav@Info@meta_df) != 0){ # Try to search for yaw in the provided meta data
        if (!anyNA(thermal_uav@Info@meta_df$Yaw)){ # If the data is complete use this as Yaw
          ...
          if (thermal_uav@Info@camera_info$tag_yaw %in% names(thermal_uav@Info@exif)){
            if (any(as.numeric(dplyr::select(thermal_uav@Info@exif, thermal_uav@Info@camera_info$tag_yaw)[[1]]))){
              thermal_uav@Position@Yaw <- as.numeric(dplyr::select(thermal_uav@Info@exif, thermal_uav@Info@camera_info$tag_yaw)[[1]])
            ...
            stop(paste0("'tag_yaw' in camera info not found in exif data, please provide the correct tag \n"))
          }
        }
      } else {
        if (thermal_uav@Info@camera_info$tag_yaw %in% names(thermal_uav@Info@exif)){
      ...
      for (i in 1:thermal_uav@Info@dataset_length){ # Loop over all the images
        ...
        imageH_m <- (as.numeric(thermal_uav@Position@FH[i])*thermal_uav@Info@camera_info$sensor_height)/thermal_uav@Info@camera_info$focal_length_def
        imageW_m <- (as.numeric(thermal_uav@Position@FH[i])*thermal_uav@Info@camera_info$sensor_width)/thermal_uav@Info@camera_info$focal_length_def
...
#'
#' @description
#' \code{\link[=tuav_view]{tuav_view()}} shows the extents or camera positions in the viewer as interactive map. NOTE: the function \code{\link[=tuav_loc]{tuav_loc()}} should have been run previously
#'
#' @param thermal_uav \code{\link[=ThermalUAV-Class]{ThermalUAV}} object retrieved by previous functions (e.g.: \code{\link[=tuav_create]{tuav_create()}})
#' @param extent (logical) use TRUE if you want to plot the image extents. NOTE: only if extents are previously calculated in the \code{\link[=tuav_loc]{tuav_loc()}} function
#' @return shows the GPS locations or the extents in an interactive way
#' @seealso \code{\link[=tuav_loc]{tuav_loc()}}
#' @export
tuav_view <- function(thermal_uav,
                      extent = FALSE){
    ...
    suppressMessages({
      ...
      if (extent == TRUE){
        if (length(terra::unwrap(thermal_uav@Position@extents_vector)) != 0){
          ...
          stop(paste0("No position shapefile found, please run the function tuav_loc \n"))
        }
      } else {
...
#'
#' @description
#' \code{\link[=coreg_prep]{coreg_prep()}} prepares the data from a different camera (with higher GPS accuracy) to use in the \code{\link[=tuav_coreg]{tuav_coreg()}} function.
#'
#' @param img_path the folder containing the high resolution images
#' @param SfM_option if the cameras were first aligned in a SfM software, for now the only option is "Agisoft Metashape". Use NA if this was not the case and you want to run it without previous alignement. Default is NA
...
#' @param camera_name The name of the high resolution camera of which the optimized GPS data will be used
...
#' @param label needed for multispectral data, it is the subscript/sequence describing the band/camera on which every position offset is calculated (in case of the Micasence Altum-PT it is the green band: "_2"). If no label enter ''
...
#' @return This function returns a data.frame containing positional information about optimized cameras which can be used as input in the \code{\link[=tuav_coreg]{tuav_coreg()}} function
#' @seealso \code{\link[=tuav_coreg]{tuav_coreg()}}
#' @export
coreg_prep <- function(img_path,
                       SfM_option = NA,
                       opt_camera_path = NA,
                       epsg = "4326",
  ...
  # Check which camera is used and extract necessary info
  # camera_info <- utils::read.csv("cameras/cameras.csv")
  check_camera <- camera_name %in% camera_info$camera_name
  if (!check_camera){
    stop(paste("camera not available in camera list \nPlease add your camera with the characteristics in the camera info file"))
  }
  camera_info <- camera_info[camera_info$camera_name == camera_name,]
  # Extract exif data
  if (!substr(img_path, nchar(img_path), nchar(img_path)) == "/") {
    img_path <- paste0(img_path, '/')
  }
  message("Reading Exif data of optimized cameras ")
  opt_cameras_exif <- exiftoolr::exif_read(list.files(path = img_path, pattern = paste0('*.',camera_info$filetype), full.names = TRUE))
  ...
  if (SfM_option == "Agisoft Metashape"){
    ...
    opt_cameras_filtered <- opt_cameras_exif[grepl(label, opt_cameras_exif$FileName),
                                             c("FileName", camera_info$tag_lon, camera_info$tag_lat, camera_info$tag_elev,
                                               camera_info$tag_yaw, camera_info$tag_pitch, camera_info$tag_roll)]
  ...
  if (nchar(camera_info$date_time) == 0 | is.na(camera_info$date_time)){
    stop(paste0("date_time is not provided in the camera info file. Please provide the right date_time label \n"))
  }
  if (!(camera_info$date_time %in% names(opt_cameras_exif))){
    stop(paste0("date_time label was not found in the exifdata of the camera. Please provide the right date_time label \n"))
  }
  datetime <- as.POSIXct(opt_cameras_exif[opt_cameras_exif$FileName %in% opt_cameras_filtered$Label, camera_info$date_time],
                         tz = timezone, tryFormats = c("%Y-%m-%d %H:%M:%OS", "%Y/%m/%d %H:%M:%OS", "%Y:%m:%d %H:%M:%OS"))
  # In case SubSecTime is provided
  if (nchar(camera_info$SubSecTime) != 0 & !(is.na(camera_info$SubSecTime))){
    if (!(camera_info$SubSecTime %in% names(opt_cameras_exif))){
      ...
      SubSecTime <- opt_cameras_exif[opt_cameras_exif$FileName %in% opt_cameras_filtered$Label, camera_info$SubSecTime]
    }
  } else {
    SubSecTime <- rep(0, length(datetime))
    warning("SubSecTime is not provided/found, can have negative implications in tuav_coreg function. \nIf flown at relative high speed, consider carefully if tuav_coreg improves the data...")
  }
  ...
  # Now we have alle the necessary info about the optimized camera dataset as dataframe. export to provide in tuav_coreg function
...
#'
#' @description
#' \code{\link[=tuav_coreg]{tuav_coreg()}} corrects the GPS-info of the thermal data with more accurate GPS information of a coregistered (high resolution) camera. Could also be that the cameras of this coregistered camera are already aligned in a structure form motion software.
#'
#' @param thermal_uav \code{\link[=ThermalUAV-Class]{ThermalUAV}} object retrieved by previous functions (e.g.: \code{\link[=tuav_create]{tuav_create()}}).
#' @param opt_cameras (data.frame) containing the optimized camera positions. In the right format, can be prepared with the \code{\link[=coreg_prep]{coreg_prep()}} function
...
#' @param timediff (numerical) if there is a time difference (in seconds) between the optimized camera set and the thermal data, it can be specified here, default is 0
#' @return This function returns an updated \code{\link[=ThermalUAV-Class]{ThermalUAV}} object containing optimized thermal camera positions
#' @seealso \code{\link[=coreg_prep]{coreg_prep()}}
#' @export
tuav_coreg <- function(thermal_uav,
                       opt_cameras = NA,
  ...
  suppressWarnings({
    ...
    if (anyNA(opt_cameras)){
      stop(paste0("opt_cameras not found, please provide opt_cameras, which can be obtained by running the 'coreg_prep()' function \n"))
    }
    ...
    thermal_uav@Position@tuav_coreg <- "Yes"
    return(thermal_uav)
  })
}
...
#' CRS function
#' @description
#' in order to plot the data with the terra::plet function, the crs(x) function should be ran locally, could be due to a bad call of CRS in the leaflet package
#' See terra::crs for more information about the function.
#' @keywords internal
